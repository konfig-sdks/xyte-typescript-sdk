...
import { readableStreamToString, XyteError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new XyteError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new XyteError(e, e.response?.data)
                }
                if (innerError instanceof XyteError) {
                    // Got "XyteError" from the above try block
                    throw innerError;
                }
